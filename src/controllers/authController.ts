...
export const register = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    logger.info('User registered successfully:', { userId: usuario.id, email: usuario.email });
...
export const login = asyncHandler(async (req: Request, res: Response) => {
  ...
  logger.info('Password comparison result:', { 
    ...
    hashLength: usuario.password_hash?.length,
  });

  if (!isValidPassword) {
    logger.warn('Invalid password:', { email: data.email });
    throw new AppError(401, 'INVALID_CREDENTIALS', 'Invalid email or password');
  }

  if (!usuario.activo) {
    logger.warn('User inactive:', { userId: usuario.id, email: data.email });
  ...
  logger.info('User logged in successfully:', { userId: usuario.id, email: usuario.email });
